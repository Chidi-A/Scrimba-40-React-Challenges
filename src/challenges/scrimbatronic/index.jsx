...
export default function ChallengeScrimbatronic() {
  ...
  function handleMouseDown(e) {
    const note = e.target.dataset.note; // e.g. 'c2', 'd-sharp'
    if (!note) return;
    ...
    setSynthKeys((prevKeys) =>
      prevKeys.map(
        (key) =>
          key.keyName === note
            ? { ...key, active: true } // play this key
            : key, // leave others unchanged
      ),
    );
  }
  ...
  function handleKeyUp(e) {
    ...
    setSynthKeys((prevKeys) =>
      prevKeys.map((key) =>
        key.computerKey === released ? { ...key, keyPressed: false } : key,
      ),
    );
  }
