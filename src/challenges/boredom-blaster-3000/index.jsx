...
*/
  ...
  const savedActivityKeys = React.useMemo(() => {
    return [
  ...
  React.useEffect(() => {
    const fetches = savedActivityKeys.map((key) =>
      fetch(`https://apis.scrimba.com/bored/api/activity?key=${key}`).then(
        (r) => r.json(),
      ),
    );
    Promise.all(fetches)
      .then((results) => setActivitiesData(results))
  ...
  return (
    <div className="boredom-blaster-3000">
