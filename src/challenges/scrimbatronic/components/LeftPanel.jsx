...
export default function LeftPanel(props) {
  ...
  const instrumentButtons = instruments.map((instrument) => {
    const activeClass =
      instrument === currentInstrument ? 'active' : 'inactive';
  ...
  if (showKeys.noKeys) {
    imageToShow = <img className="notes-icon" src="./music-solid.svg" />;
  } else if (showKeys.musicKeys) {
    imageToShow = <img className="keyboard-icon" src="./keyboard-solid.svg" />;
  } else {
    imageToShow = (
      <img className="blank-icon" src="./circle-xmark-regular.svg" />
    );
  }
  ...
  return (
    <div className="left-panel-container" onClick={handleClick}>
      ...
      <div className="little-buttons-container">
        ...
        >
          <img className="metronome-icon" src="./metronome.png" />
        </button>
      </div>
